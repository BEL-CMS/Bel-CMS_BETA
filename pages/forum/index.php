		...
		foreach ($forum as $k => $v):
			...
			<?php
				foreach ($v->threads as $k_threads => $v_threads):
					...
					$none   = $v_threads->lastThreads->title == NO_POST ? '<h4 style="line-height:35px">'.NO_POST.'</h4>' : '<h4><a href="Forum/Post/'.$v_threads->lastThreads->title.'/'.$v_threads->lastThreads->id.'">'.$v_threads->lastThreads->title.'</a></h4>';
				?>
