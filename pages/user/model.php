...
class ModelPagesUser
{
	...
	#####################################
	protected function getDataUser ($hash_key = false)
	{
	...
	# Insert registration
	#####################################
	protected function sendRegistration (array $data)
	{
		if ($data) {
			$error = null;
	...
	protected function sendEditProfil ($data) {
		$error  = true;
		$insertProfil = array();
		...
		if ($countRowUpdate != 0) {
			...
			$return['msg']  = 'Vos informations n\'ont pas été sauvegardées';
			$return['type'] = 'blue';
		}

		return $return;
	}
	protected function sendEditSocial ($data) {
	...
	protected function sendEditPassword ($data) {
		$insertUser   = array();
		$insertProfil = array();
		$error        = true;
		...
		if ($error && $data['public_mail'] != $resultsProfils->public_mail) {
			...
			if (!filter_var($data['email'], FILTER_VALIDATE_EMAIL)) {
				$return['msg']  = 'le courriel '.$data['email'].' n\'est pas valide';
				$return['type'] = 'danger';
			} else if (in_array($tmpNdd[0], $arrayBlackList)) {
				$return['msg']  = 'Les faux mails ne sont pas autorisés';
				$return['type'] = 'danger';
	...
	protected function sendAvatarUpload ()
	{
	...
	protected function sendChangeAvatar ($data = false)
	{
		$rowCount = null;
		if ($data) {
			$dir = 'uploads/users/'.$_SESSION['user']->hash_key.'/';
		...
		if ($rowCount === null) {
			...
			$file = ROOT_CONFIG.'users.ini';
			$ini = New iniParser($file);
			$ini->set('AVATAR', $_SESSION['user']->hash_key,$data);
			$ini->save($file);
	...
	protected function sendDeleteAvatar ($data = false)
	{
		if ($data) {
			$dir = 'uploads/users/'.$_SESSION['user']->hash_key.'/';
			$checkdir = strpos($data, $dir);
